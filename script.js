...
function checkTime() {
  if(playerGuessArray.length == questionAmount) {
    console.log('arrey of player guess', playerGuessArray);
    clearInterval((timer));
...
//scroll, store user selcrtion in playerGuessArray
function select(guessTrue) {
  console.log('guess', guessTrue);
  //scroll 80px
  valueY += 80;
...
function createEquations() {
  ...
  console.log('Correcte Euation',correctEquations);
...
startForm.addEventListener('click', () => {
  radioContainers.forEach((radioEL) => {
    //Remove slected label
    radioEL.classList.remove('selected-label');
    //add back if radio input check
